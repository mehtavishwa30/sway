...
impl FnCompiler {
    ...
    fn compile_lazy_op(
        ...
        let merge_val_arg_idx = final_block.new_arg(
            ...
            lhs_val
                .get_type(context)
                .unwrap_or_else(|| rhs_val.get_type(context).unwrap_or(Type::get_unit(context))),
        );
    ...
    fn compile_contract_call(
        ...
        let user_args_val = match compiled_args.len() {
            ...
            _ => {
                ...
                let field_types = compiled_args
                    ...
                    .collect::<Vec<_>>();
                let user_args_struct_aggregate = Type::get_struct(context, field_types);
        ...
        let ra_struct_aggregate = Type::get_struct(
            context,
            [
        ...
        let gep = self
            .current_block
            .ins(context)
            .get_elm_ptr_from_int_idx(ra_struct_val, ra_struct_aggregate, vec![0, 0])
            .add_metadatum(context, span_md_idx);
        self.current_block
            .ins(context)
        ...
        let gep = self
            .current_block
            .ins(context)
            .get_elm_ptr_from_int_idx(ra_struct_val, ra_struct_aggregate, vec![0, 1])
            .add_metadatum(context, span_md_idx);
        self.current_block
            .ins(context)
            .store(gep, sel_val)
        ...
        let gep = self
            .current_block
            .ins(context)
            .get_elm_ptr_from_int_idx(ra_struct_val, ra_struct_aggregate, vec![0, 2])
            .add_metadatum(context, span_md_idx);
        self.current_block
            .ins(context)
            .store(gep, user_args_val)
            .add_metadatum(context, span_md_idx);
    ...
    fn compile_fn_call(
        ...
        if args.len() + 1 == new_callee.num_args(context) {
            if let Some(ptr_type) = new_callee
                .args_iter(context)
                .last()
                .unwrap()
                .1
                .get_argument_type(context)
                .and_then(|t| t.get_inner_ptr_type(context))
            {
    ...
    fn compile_if(
        ...
        let merge_val_arg_idx = merge_block.new_arg(
            ...
            true_value.get_type(context).unwrap_or_else(|| {
                false_value
                    .get_type(context)
                    .unwrap_or(Type::get_unit(context))
            }),
        );
    ...
    fn compile_unsafe_downcast(
        ...
        let enum_aggregate = convert_resolved_typeid(context, &exp.return_type, &exp.span)?;
    ...
    fn compile_enum_tag(
        ...
        let gep = self
            .current_block
            .ins(context)
            .get_elm_ptr_from_int_idx(exp, enum_aggregate, vec![0, 0])
    ...
    fn compile_var_expr(
            ...
            .and_then(|local_name| self.function.get_local_ptr(context, local_name))
        {
            ...
            if arg_ty.is_ptr_type(context) && arg_ty.strip_ptr_type(context).is_copy_type(context) {
                Ok(self
                    .current_block
                    .ins(context)
                    .load(val)
    ...
    fn compile_var_decl(
        ...
        if ir_type_size_in_bytes(context, &ptr_ty) > 0 {
            let ptr_val = self
                ...
                .get_ptr(ptr, ptr_ty, 0)
                .add_metadatum(context, span_md_idx);
            if ptr_ty.strip_ptr_type(context).is_copy_type(context) {
                self.current_block
                    .ins(context)
    ...
    fn compile_reassignment(
        ...
        if ast_reassignment.lhs_indices.is_empty() {
            ...
            let ty = val.get_stripped_ptr_type(context).unwrap();
            ...
            let gep = self
                .current_block
                .ins(context)
                .get_elm_ptr_from_int_idx(val, ty, [vec![0], field_idcs].concat())
                .add_metadatum(context, span_md_idx);
            self.current_block
                .ins(context)
                .store(gep, reassign_val)
                .add_metadatum(context, span_md_idx);
        }
    ...
    fn compile_array_expr(
        ...
        for (idx, elem_expr) in contents.into_iter().enumerate() {
            ...
            let gep = self
                ...
                .add_metadatum(context, span_md_idx);
            if elem_type.is_copy_type(context) {
                self.current_block
                    .ins(context)
    ...
    fn compile_array_index(
        ...
        } else if let Some(agg_ty) = array_val.get_argument_type(context) {
    ...
    fn compile_struct_expr(
        ...
        for (insert_val, insert_idx) in inserted_values_indices {
            let gep = self
                .current_block
                .ins(context)
            ...
            let indexed_type = gep
                ...
                .unwrap();
            if indexed_type.is_copy_type(context) {
                self.current_block
                    .ins(context)
    ...
    fn compile_enum_expr(
        ...
        let tag_gep = self
            .current_block
            .ins(context)
            .get_elm_ptr_from_int_idx(enum_ptr_value, aggregate, vec![0, 0])
            .add_metadatum(context, span_md_idx);
        self.current_block.ins(context).store(tag_gep, tag_value);
        ...
        Ok(if has_single_field_type {
            ...
            let contents_value = contents
                .map(|te| self.compile_expression(context, md_mgr, *te))
                .transpose()?;

            match contents_value {
                None => enum_ptr_value,
                Some(te) => {
                    ...
                    let te_type = te.get_type(context).ok_or_else(|| {
                        CompileError::Internal("Enum initialiser must have a type.", enum_decl.span)
                    })?;

                    if te_type.is_copy_type(context) {
        ...
        if fields.is_empty() {
                ...
                init_values.into_iter().zip(init_types).enumerate()
            {
                let gep = self
                    ...
                    .add_metadatum(context, span_md_idx);
                if insert_type.is_copy_type(context) {
    ...
    fn compile_tuple_elem_expr(
        ...
        let aggregate = convert_resolved_typeid(context, &tuple_type, &span)?;
        if aggregate.is_struct(context) {
            let span_md_idx = md_mgr.span_to_md(context, &span);
            let gep = self
                .current_block
                .ins(context)
                .get_elm_ptr_from_int_idx(tuple_value, aggregate, vec![0, idx as u64])
                .add_metadatum(context, span_md_idx);
            Ok(self.current_block.ins(context).load(gep))
        } else {
        ...
        if let TypeContent::Struct(fields) = &content {
            ...
            for (field_idx, field_type) in fields.into_iter().enumerate() {
                ...
                let gep = self
                    ...
                    .add_metadatum(context, span_md_idx);
                if field_type.is_copy_type(context) {
        ...
        if let TypeContent::Struct(fields) = &content {
            for (field_idx, field_type) in fields.into_iter().enumerate() {
                ...
                let extracted_value = if gep
                    .get_type(context)
                    .map(|ty| ty.strip_ptr_type(context).is_copy_type(context))
                    .unwrap_or(false)
                {
    ...
    fn compile_union_or_string_storage_read(
        ...
        let b256_array_type = Type::get_array(context, Type::get_b256(context), number_of_elements);
    ...
    fn compile_union_or_string_storage_write(
        ...
        // Store the value to the local pointer created for rhs
        if r#type.is_copy_type(context) {
            self.current_block
                .ins(context)
