...
impl<'ir> AsmBuilder<'ir> {
    ...
    fn compile_instruction(&mut self, instr_val: &Value, func_is_entry: bool) -> CompileResult<()> {
        ...
        if let Some(instruction) = instr_val.get_instruction(self.context) {
            match instruction {
                ...
                Instruction::Nop => (),
                Instruction::PtrToInt(ptr_val) => self.compile_ptr_to_int(instr_val, ptr_val),
    ...
    fn compile_log(&mut self, instr_val: &Value, log_val: &Value, log_ty: &Type, log_id: &Value) {
        ...
        // XXX again, ref-type vals need to have pointer types.
        if !log_ty.is_ptr_type(self.context) {
            self.cur_bytecode.push(Op {
                owning_span,
            ...
            // If the type is a reference type then we use LOGD to log the data. First put the
    ...
    fn compile_ptr_to_int(&mut self, instr_val: &Value, ptr_val: &Value) {
        // This is a no-op, assuming the IR is valid/verified.
        let val_reg = self.value_to_register(ptr_val);
        self.reg_map.insert(*instr_val, val_reg);
    }
    ...
    fn compile_ret_from_entry(&mut self, instr_val: &Value, ret_val: &Value, ret_type: &Type) {
        let owning_span = self.md_mgr.val_to_span(self.context, *instr_val);
        if ret_type.eq(self.context, &Type::get_unit(self.context)) {
            ...
            if !ret_type.is_ptr_type(self.context) {
                self.cur_bytecode.push(Op {
                    owning_span,
                ...
                // Use RETD to return data.  First put the size into the data section, then add a
                // LW to get it, then add a RETD which uses it.
                let size_reg = self.reg_seqr.next();
    ...
    fn compile_state_access_quad_word(
        ...
        assert!(offset == 0);
        assert!(ptr_ty.eq(self.context, &Type::get_b256(self.context)));
