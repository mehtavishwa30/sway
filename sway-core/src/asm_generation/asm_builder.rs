...
impl<'ir> AsmBuilder<'ir> {
    ...
    fn compile_instruction(&mut self, instr_val: &Value, func_is_entry: bool) -> CompileResult<()> {
        ...
        if let Some(instruction) = instr_val.get_instruction(self.context) {
            match instruction {
                Instruction::AddrOf(arg, _ty) => self.compile_addr_of(instr_val, arg),
                ...
                Instruction::ExtractElement {
                    ...
                    array_ty,
                    index_val,
                } => self.compile_extract_element(instr_val, array, array_ty, index_val),
                ...
                Instruction::FuelVm(fuel_vm_instr) => match fuel_vm_instr {
                    ...
                    FuelVmInstruction::ReadRegister(reg, _ty) => {
                    ...
                    FuelVmInstruction::StateLoadWord(key, _ty) => check!(
                ...
                Instruction::InsertElement {
                    ...
                    value,
                    index_val,
                } => self.compile_insert_element(instr_val, array, array_ty, value, index_val),
    ...
    fn compile_log(&mut self, instr_val: &Value, log_val: &Value, log_ty: &Type, log_id: &Value) {
        ...
        if log_ty.is_copy_type(self.context) {
            self.cur_bytecode.push(Op {
                owning_span,
    ...
    fn compile_ret_from_entry(&mut self, instr_val: &Value, ret_val: &Value, ret_type: &Type) {
        ...
        if ret_type.is_unit(self.context) {
            ...
            if ret_type.is_copy_type(self.context) {
                self.cur_bytecode.push(Op {
                    owning_span,
